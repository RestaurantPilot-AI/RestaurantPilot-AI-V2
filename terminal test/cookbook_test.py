...
from src.processing.cookbook_ingestion import extract_cookbook
...
def run_pipeline():
    ...
    for file_path in STAGING_DIR.iterdir():
        ...
        try:
            cookbook_df = extract_cookbook(temp_restaurant_id, file_path)
            ...
            # Print full traceback to ease debugging
            traceback.print_exc()
            # Move to processed to avoid blocking pipeline
            shutil.move(str(file_path), PROCESSED_DIR / file_path.name)
            continue

        # Save using DataFrame-only signature; save_cookbook_db will use restaurant_id inside df
        bs = db.save_cookbook_db(cookbook_df)
        print(f"Saved: {file_path} -> {bs}")
